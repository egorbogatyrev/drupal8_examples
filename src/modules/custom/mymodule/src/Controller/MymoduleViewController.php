...
/**
 * Defines a controller to render a single mymodule.
 */
class MymoduleViewController extends EntityViewController {
  ...
  /**
   * Creates an MymoduleViewController object.
   *
   ...
   * {@inheritdoc}
   */
  public function view(EntityInterface $mymodule, $view_mode = 'full', $langcode = NULL) {
    $build = parent::view($mymodule, $view_mode, $langcode);

    foreach ($mymodule->uriRelationships() as $rel) {
      $url = $mymodule->toUrl($rel);
      ...
      if ($this->currentUser->isAuthenticated() || $url->access($this->currentUser)) {
        // Set the mymodule path as the canonical URL to prevent duplicate content.
  ...
  /**
   * The _title_callback for the page that renders a single mymodule.
   *
   * @param \Drupal\Core\Entity\EntityInterface $mymodule
   *   The current mymodule.
   *
   * @return string
   *   The page title.
   */
  public function title(EntityInterface $mymodule) {
    return $this->entityManager->getTranslationFromContext($mymodule)->label();
  }
